import numpy as np
# from numba import njit
import matplotlib.pyplot as plt
from scipy.integrate import solve_ivp

# Initial conditions
Atmosphere_Initial = 750
CarbonateRock_Initial = 100000000
DeepOcean_Initial = 38000
FossilFuel_Initial = 7500
Plant_Initial = 560
Soil_Initial = 1500
SurfaceOcean_Initial = 890
VegLandArea_percent_Initial = 100

x0 = np.array([Atmosphere_Initial,
                CarbonateRock_Initial,
                DeepOcean_Initial,
                FossilFuel_Initial,
                Plant_Initial,
                Soil_Initial,
                SurfaceOcean_Initial,
                VegLandArea_percent_Initial
                ], dtype=float)

# Constants
Alk = 2.222446077610055
Kao = .278
SurfOcVol = .0362
Deforestation = 0

# Helper functions
# @njit
def AtmCO2(Atmosphere):
    return Atmosphere * (280/Atmosphere_Initial)
# @njit
def GlobalTemp(AtmCO2):
    return 15 + ((AtmCO2-280) * .01)
# @njit
def CO2Effect(AtmCO2):
    return 1.5 * ((AtmCO2) - 40) / ((AtmCO2) + 80)
# @njit
def WaterTemp(GlobalTemp):
    return 273+GlobalTemp
# @njit
def TempEffect(GlobalTemp):
    return ((60 - GlobalTemp) * (GlobalTemp + 15)) / (((60 + 15) / 2) ** (2))/.96
# @njit
def SurfCConc(SurfaceOcean):
    return (SurfaceOcean/12000)/SurfOcVol
# @njit
def Kcarb(WaterTemp):
    return .000575+(.000006*(WaterTemp-278))
# @njit
def KCO2(WaterTemp):
    return .035+(.0019*(WaterTemp-278))
# @njit
def HCO3(Kcarb, SurfCConc):
    return(SurfCConc-(np.sqrt(SurfCConc**2-Alk*(2*SurfCConc-Alk)*(1-4*Kcarb))))/(1-4*Kcarb)
# @njit
def CO3(HCO3):
    return (Alk-HCO3)/2
# @njit
def pCO2Oc(KCO2, HCO3, CO3):
    return 280*KCO2*(HCO3**2/CO3)


# Fossil fuels
FossFuelData0 = np.array([[1850.0, 0.00], [1875.0, 0.30], [1900.0, 0.60], [1925.0, 1.35], [1950.0, 2.85], [1975.0, 4.95], [2000.0, 7.20], [2025.0, 10.05], [2050.0, 14.85], [2075.0, 20.70], [2100.0, 30.00]])
# CO2 equivalent of 10.05 Gt carbon is 36.88 Gt CO2
FossFuelData1 = np.array([[1850.0, 0.00], [1875.0, 0.30], [1900.0, 0.60], [1925.0, 1.35], [1950.0, 2.85], [1975.0, 4.95], [2000.0, 7.20], [2025.0, 10.05], [2050.0, 14.85], [2075.0, 20.70], [2100.0, 30.00], [2125.0, 45.00], [2150.0, 67.50], [2175.0, 101.25], [2200.0, 151.88]])

FossFuelData = FossFuelData0.copy()

# @njit
def FossilFuelsCombustion(t):
    i = 0
    if t >= FossFuelData[-1,0]:
        return FossFuelData[-1,1]
    while i + 1 < len(FossFuelData) and t >= FossFuelData[i,0]:
        i = i + 1
    if i == 0:
        return FossFuelData[0,1]
    else:
        return FossFuelData[i-1,1] + (t - FossFuelData[i-1,0]) / (FossFuelData[i,0] - FossFuelData[i-1,0]) * (FossFuelData[i,1] - FossFuelData[i-1,1])

# @njit
def derivative(x, t):
    Atmosphere = x[0]
    CarbonateRock = x[1]
    DeepOcean = x[2]
    FossilFuelCarbon = x[3]
    Plants = x[4]
    Soils = x[5]
    SurfaceOcean = x[6]
    VegLandArea_percent = x[7]

    PlantResp = (Plants * (55/Plant_Initial)) + Deforestation/2
    Litterfall = (Plants* (55/Plant_Initial))+(Deforestation/2)
    SoilResp = Soils * (55/Soil_Initial)
    Volcanoes = 0.1
    AtmCO2_ = AtmCO2(Atmosphere)
    GlobalTemp_ = GlobalTemp(AtmCO2_)
    WaterTemp_ = WaterTemp(GlobalTemp_)
    Photosynthesis = 110 * CO2Effect(AtmCO2_) * (VegLandArea_percent/100) * TempEffect(GlobalTemp_)
    HCO3_ = HCO3(Kcarb(WaterTemp_), SurfCConc(SurfaceOcean))
    pCO2Oc_ = pCO2Oc(KCO2(WaterTemp_), HCO3_, CO3(HCO3_))
    AtmOcExchange = Kao*(AtmCO2_-pCO2Oc_)
    if x[3] > 0:
        FossilFuelsCombustion_ = FossilFuelsCombustion(t)
    else:
        FossilFuelsCombustion_ = 0
    dAtmosphere_dt = (PlantResp + SoilResp + Volcanoes + FossilFuelsCombustion_
                            - Photosynthesis - AtmOcExchange)

    Sedimentation = DeepOcean * (0.1/DeepOcean_Initial)
    dCarbonateRock_dt = Sedimentation - Volcanoes

    Downwelling = SurfaceOcean*(90.1/SurfaceOcean_Initial)
    Upwelling = DeepOcean * (90/DeepOcean_Initial)
    dDeepOcean_dt= Downwelling - Sedimentation - Upwelling

    dFossilFuelCarbon_dt = - FossilFuelsCombustion_

    dPlants_dt = Photosynthesis - PlantResp - Litterfall

    dSoils_dt = Litterfall - SoilResp

    dSurfaceOcean_dt = Upwelling + AtmOcExchange - Downwelling

    Development = (Deforestation/Plant_Initial * 0.2) * 100
    dVegLandArea_percent_dt = - Development

    derivative = np.array([
        dAtmosphere_dt,
        dCarbonateRock_dt,
        dDeepOcean_dt,
        dFossilFuelCarbon_dt,
        dPlants_dt,
        dSoils_dt,
        dSurfaceOcean_dt,
        dVegLandArea_percent_dt
        ])

    return derivative


def f(t, x):
    return derivative(x, t)


def solve(phi, f, t0, T, x0, h):
    t = np.arange(t0, T+h, h)
    x = np.zeros((len(t), len(x0)))
    x[0] = x0.copy()
    for i in range(len(t)-1):
        x[i+1] = phi(f, t[i], x[i], h) 
    return t, x

def euler(f, t, x, h):
    return x + h * f(t, x)

def runge_kutta_2(f, t, x, h):
    p1 = f(t, x)
    p2 = f(t + h, x + h * p1)
    return x + h * (p1/2 + p2/2)

def runge_kutta_4(f, t, x, h):
    p1 = f(t, x)
    p2 = f(t + h/2, x + h/2 * p1)
    p3 = f(t + h/2, x + h/2 * p2)
    p4 = f(t + h, x + h * p3)
    return x + h * (p1/6 + 2*p2/6 + 2*p3/6 + p4/6)



if __name__ == "__main__":
    t0 = 1850
    T = 2600
    h = 0.2
    print("Lancement simulation")
    

    print("Lancement Euler")
    t_euler, x_euler = solve(euler, f, t0, T, x0, h)
    print("Lancement Runge-Kutta 2")
    t_rk2, x_rk2 = solve(runge_kutta_2, f, t0, T, x0, h)
    print("Lancement Runge-Kutta 4")
    t_rk4, x_rk4 = solve(runge_kutta_4, f, t0, T, x0, h)

    print(f"  CO2 final Euler : {x_euler[-1, 0] :.1f} ppm")
    print(f"  CO2 final RK2   : {x_rk2[-1, 0] :.1f} ppm")
    print(f"  CO2 final RK4   : {x_rk4[-1, 0] :.1f} ppm")
    
    ref = solve_ivp(f, (t0, T), x0, method='DOP853', rtol=1e-12, atol=1e-12, dense_output=True)
    x_final_ref = ref.sol(T)
    print(f"  CO2 final Ref   : {x_final_ref[0] :.1f} ppm")
        
    fig, ax1 = plt.subplots(figsize=(10, 5))
    ax1.plot(t_euler, x_euler[:, 4], color='green', label="Plants")
    ax1.set_xlabel("Year")
    ax1.set_ylabel("Plants", color='green')
    ax1.tick_params(axis='y', labelcolor='green')
    ax2 = ax1.twinx()
    ax2.plot(t_euler, x_euler[:, 5], color='brown', label="Soils")
    ax2.set_ylabel("Soils", color='brown')
    ax2.tick_params(axis='y', labelcolor='brown')
    ax3 = ax1.twinx()
    ax3.plot(t_euler, x_euler[:, 0], color='blue', label="Atmosphere")
    ax3.set_ylabel("Atmosphere", color='blue')
    ax3.tick_params(axis='y', labelcolor='blue')
    ax4 = ax1.twinx()
    ax4.plot(t_euler, x_euler[:, 3], color='orange', label="Fossil Fuel Carbon")
    ax4.set_ylabel("Fossil Fuel Carbon", color='orange')
    ax4.tick_params(axis='y', labelcolor='orange')
    ax5 = ax1.twinx()
    ax5.plot(t_euler, x_euler[:, 7], color='purple', label="Veg Land Area %")
    ax5.set_ylabel("Veg Land Area %", color='purple')
    ax5.tick_params(axis='y', labelcolor='purple')
    ax6 = ax1.twinx()
    #Surface Ocean diverge for h = 0.2 it corrupts the graph 
    # So need to be commented if h is too large 
    ax6.plot(t_euler, x_euler[:, 6], color='cyan', label="Surface Ocean")
    ax6.set_ylabel("Surface Ocean", color='cyan')
    ax6.tick_params(axis='y', labelcolor='cyan')
    ax7 = ax1.twinx()
    ax7.plot(t_euler, x_euler[:, 2], color='magenta', label="Deep Ocean")
    ax7.set_ylabel("Deep Ocean", color='magenta')
    ax7.tick_params(axis='y', labelcolor='magenta')
    ax8 = ax1.twinx()
    ax8.plot(t_euler, x_euler[:, 1], color='gray', label="Carbonate Rock")
    ax8.set_ylabel("Carbonate Rock", color='gray')
    ax8.tick_params(axis='y', labelcolor='gray')
    plt.title("Carbon Cycle Simulation (Euler)")
    lines1, labels1 = ax1.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    lines3, labels3 = ax3.get_legend_handles_labels()
    lines4, labels4 = ax4.get_legend_handles_labels()
    lines5, labels5 = ax5.get_legend_handles_labels()
    lines6, labels6 = ax6.get_legend_handles_labels()
    lines7, labels7 = ax7.get_legend_handles_labels()
    lines8, labels8 = ax8.get_legend_handles_labels()
    ax1.legend(lines1 + lines2 + lines3 + lines4 + lines5 + lines6 + lines7 + lines8, labels1 + labels2 + labels3 + labels4 + labels5 + labels6 + labels7 + labels8, loc='best')
    plt.show()
    
    
    fig, ax1 = plt.subplots(figsize=(10, 5))
    ax1.plot(t_rk2, x_rk2[:, 4], color='green', label="Plants")
    ax1.set_xlabel("Year")
    ax1.set_ylabel("Plants", color='green')
    ax1.tick_params(axis='y', labelcolor='green')
    ax2 = ax1.twinx()
    ax2.plot(t_rk2, x_rk2[:, 5], color='brown', label="Soils")
    ax2.set_ylabel("Soils", color='brown')
    ax2.tick_params(axis='y', labelcolor='brown')
    ax3 = ax1.twinx()
    ax3.plot(t_rk2, x_rk2[:, 0], color='blue', label="Atmosphere")
    ax3.set_ylabel("Atmosphere", color='blue')
    ax3.tick_params(axis='y', labelcolor='blue')
    ax4 = ax1.twinx()
    ax4.plot(t_rk2, x_rk2[:, 3], color='orange', label="Fossil Fuel Carbon")
    ax4.set_ylabel("Fossil Fuel Carbon", color='orange')
    ax4.tick_params(axis='y', labelcolor='orange')
    ax5 = ax1.twinx()
    ax5.plot(t_rk2, x_rk2[:, 7], color='purple', label="Veg Land Area %")
    ax5.set_ylabel("Veg Land Area %", color='purple')
    ax5.tick_params(axis='y', labelcolor='purple')
    ax6 = ax1.twinx()
    ax6.plot(t_rk2, x_rk2[:, 6], color='cyan', label="Surface Ocean")
    ax6.set_ylabel("Surface Ocean", color='cyan')
    ax6.tick_params(axis='y', labelcolor='cyan')
    ax7 = ax1.twinx()
    ax7.plot(t_rk2, x_rk2[:, 2], color='magenta', label="Deep Ocean")
    ax7.set_ylabel("Deep Ocean", color='magenta')
    ax7.tick_params(axis='y', labelcolor='magenta')
    ax8 = ax1.twinx()
    ax8.plot(t_rk2, x_rk2[:, 1], color='gray', label="Carbonate Rock")
    ax8.set_ylabel("Carbonate Rock", color='gray')
    ax8.tick_params(axis='y', labelcolor='gray')
    plt.title("Carbon Cycle Simulation (Runge-Kutta 2)")
    lines1, labels1 = ax1.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    lines3, labels3 = ax3.get_legend_handles_labels()
    lines4, labels4 = ax4.get_legend_handles_labels()
    lines5, labels5 = ax5.get_legend_handles_labels()
    lines6, labels6 = ax6.get_legend_handles_labels()
    lines7, labels7 = ax7.get_legend_handles_labels()
    lines8, labels8 = ax8.get_legend_handles_labels()
    ax1.legend(lines1 + lines2 + lines3 + lines4 + lines5 + lines6 + lines7 + lines8, labels1 + labels2 + labels3 + labels4 + labels5 + labels6 + labels7 + labels8, loc='best')
    plt.show()
    
    
    fig, ax1 = plt.subplots(figsize=(10, 5))
    ax1.plot(t_rk4, x_rk4[:, 4], color='green', label="Plants")
    ax1.set_xlabel("Year")
    ax1.set_ylabel("Plants", color='green')
    ax1.tick_params(axis='y', labelcolor='green')
    ax2 = ax1.twinx()
    ax2.plot(t_rk4, x_rk4[:, 5], color='brown', label="Soils")
    ax2.set_ylabel("Soils", color='brown')
    ax2.tick_params(axis='y', labelcolor='brown')
    ax3 = ax1.twinx()
    ax3.plot(t_rk4, x_rk4[:, 0], color='blue', label="Atmosphere")
    ax3.set_ylabel("Atmosphere", color='blue')
    ax3.tick_params(axis='y', labelcolor='blue')
    ax4 = ax1.twinx()
    ax4.plot(t_rk4, x_rk4[:, 3], color='orange', label="Fossil Fuel Carbon")
    ax4.set_ylabel("Fossil Fuel Carbon", color='orange')
    ax4.tick_params(axis='y', labelcolor='orange')
    ax5 = ax1.twinx()
    ax5.plot(t_rk4, x_rk4[:, 7], color='purple', label="Veg Land Area %")
    ax5.set_ylabel("Veg Land Area %", color='purple')
    ax5.tick_params(axis='y', labelcolor='purple')
    ax6 = ax1.twinx()
    ax6.plot(t_rk4, x_rk4[:, 6], color='cyan', label="Surface Ocean")
    ax6.set_ylabel("Surface Ocean", color='cyan')
    ax6.tick_params(axis='y', labelcolor='cyan')
    ax7 = ax1.twinx()
    ax7.plot(t_rk4, x_rk4[:, 2], color='magenta', label="Deep Ocean")
    ax7.set_ylabel("Deep Ocean", color='magenta')
    ax7.tick_params(axis='y', labelcolor='magenta')
    ax8 = ax1.twinx()
    ax8.plot(t_rk4, x_rk4[:, 1], color='gray', label="Carbonate Rock")
    ax8.set_ylabel("Carbonate Rock", color='gray')
    ax8.tick_params(axis='y', labelcolor='gray')
    plt.title("Carbon Cycle Simulation (Runge-Kutta 4)")
    lines1, labels1 = ax1.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    lines3, labels3 = ax3.get_legend_handles_labels()
    lines4, labels4 = ax4.get_legend_handles_labels()
    lines5, labels5 = ax5.get_legend_handles_labels()
    lines6, labels6 = ax6.get_legend_handles_labels()
    lines7, labels7 = ax7.get_legend_handles_labels()
    lines8, labels8 = ax8.get_legend_handles_labels()
    ax1.legend(lines1 + lines2 + lines3 + lines4 + lines5 + lines6 + lines7 + lines8, labels1 + labels2 + labels3 + labels4 + labels5 + labels6 + labels7 + labels8, loc='best')
    plt.show()
    
    
    #Graphique pour l'ordre (Erreur en fct de h)
    h_values = [0.05, 0.01, 0.005, 0.001, 0.0005]
    errors_euler = []
    errors_rk2 = [] 
    errors_rk4 = []
        
    for h in h_values:
        print(f"Calcul pour h={h}" )
        t_euler_h, x_euler_h = solve(euler, f, t0, T, x0, h)
        t_rk2_h, x_rk2_h = solve(runge_kutta_2, f, t0, T, x0, h)
        t_rk4_h, x_rk4_h = solve(runge_kutta_4, f, t0, T, x0, h)
        errors_euler.append(np.linalg.norm(x_euler_h[-1, :] - x_final_ref[:]))
        errors_rk2.append(np.linalg.norm(x_rk2_h[-1, :] - x_final_ref[:]))
        errors_rk4.append(np.linalg.norm(x_rk4_h[-1, :] - x_final_ref[:])) 
    plt.figure(figsize=(10, 5))
    plt.plot(h_values, errors_euler, label="Euler", marker='o')
    plt.plot(h_values, errors_rk2, label="Runge-Kutta 2", marker='o')
    plt.plot(h_values, errors_rk4, label="Runge-Kutta 4", marker='o')
    plt.xlabel("Step size (h)")
    plt.ylabel("Error in final CO2 concentration")
    plt.title("Error in Final CO2 Concentration vs Step Size")
    plt.xscale('log')
    plt.yscale('log')
    plt.legend()
    plt.show()
    
    #Graphique erreur en fct du temps
    
    def erreur_temps(t, x_num, x_ref):
        return [np.linalg.norm(x_num[n, :] - x_ref(t[n]), axis=0 ) for n in range(len(t))]
    
    plt.figure(figsize=(10, 5))
    
    err_euler = erreur_temps(t_euler, x_euler, ref.sol)
    err_rk2 = erreur_temps(t_rk2, x_rk2, ref.sol)
    err_rk4 = erreur_temps(t_rk4, x_rk4, ref.sol)
    
    plt.semilogy(t_euler, err_euler, label="Euler")
    plt.semilogy(t_rk2, err_rk2, label="Runge-Kutta 2")
    plt.semilogy(t_rk4, err_rk4, label="Runge-Kutta 4")
    plt.xlabel("Year") 
    plt.ylabel("Relative Error (log scale)")
    plt.title("Error in Time")
    plt.legend()
    plt.show()
    
    
    
    
    